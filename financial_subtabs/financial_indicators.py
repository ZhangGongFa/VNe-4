"""
Financial indicators subtab for the Financial section.

This module contains utilities for computing a wide range of
financial indicators from both wide and long‑format financial data.
The results are presented as a table with one row per indicator and
columns for each year.  It is a slightly simplified copy of the
original VNe‑4 implementation with minor formatting adjustments.
"""
...
def _yearlike_columns(df: pd.DataFrame) -> List[str]:
    cols = []
    for c in df.columns:
        if re.fullmatch(r"\d{4}[A-Z]?", str(c)):
            cols.append(c)
    return cols
...
ALIASES: Dict[str, List[str]] = {
    ...
    "financial_expenses": ["financial expenses", "chi phí tài chính", "chi phi tai chinh"],
    "tax_expense": [
    ...
    "st_debt": [
        ...
        "short term loans",
        "short term debt",
        "short-term loans and financial lease payables",
    ],
    "lt_debt": [
        "long-term loans",
        "long term loans",
        "long term debt",
        "long-term loans and financial lease payables",
    ],
    "ebitda": ["ebitda"],
...
def compute_indicators(fin_df: pd.DataFrame) -> pd.DataFrame:
    """Compute a set of financial indicators from the raw financial dataframe.

    The function extracts or constructs series for each metric using
    both wide and long formats.  The resulting dataframe has years as
    the index and indicators as the columns.  An empty dataframe is
    returned if insufficient data is available.
    """
    # Base series
    ...
    ebit = _extract_series(fin_df, "ebit")
    netinc = _extract_series(fin_df, "net_income")
    ...
    df = (
        df.assign(__sort_year__=year_num, __lbl__=lbl)
...
def render(fin_df: pd.DataFrame) -> None:
