"""Visualisation helpers for default distribution.

This module provides functions that produce Plotly figures and
tables summarising default statistics by year and by sector.  The
functions are largely copied from the original project but have
minor adjustments to formatting and default colours.  They return
Plotly figure objects which can be displayed using
``st.plotly_chart``.
"""
...
def default_distribution_by_year(df: pd.DataFrame):
    """Return figures showing the distribution of defaults by year.

    Parameters
    ----------
    df : pandas.DataFrame
        DataFrame containing at least ``Year`` and ``Default`` columns.

    Returns
    -------
    tuple
        A tuple (pie_fig, bar_default_rate_year_total, table_fig_year_total).
    """
...
def default_distribution_by_sector(df: pd.DataFrame):
    """Return figures showing the distribution of defaults by sector.

    Parameters
    ----------
    df : pandas.DataFrame
        DataFrame containing ``Sector`` and ``Default`` columns.

    Returns
    -------
    tuple
        A tuple (bar_fig, pie_fig, bar_default_rate, table_fig).
    """
