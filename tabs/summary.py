...
from utils_new.model_scoring import model_feature_names, explain_shap, load_train_reference
...
def render(feats_df: pd.DataFrame, raw_df: pd.DataFrame, ticker: str, year: int,
        ...
        st.markdown(
            ...
            <div style='font-size:12px;'>
              <span style='display:inline-flex;align-items:center;gap:8px;'>
                ...
                High ≥ 50% • Floor/Cap: {pd_floor:.0%}/0.98 • Exchange: {exchange or '-'}
              </span>
            </div>
            """,
    ...
    gauge = go.Figure(go.Indicator(
        ...
        title={'text': "PD (multi-factor, post-adj.)"},
    ))
    ...
    if shap_raw is None or (hasattr(shap_raw, 'empty') and shap_raw.empty):
        ...
        # Normalise to DataFrame with columns Feature & SHAP
        if isinstance(shap_raw, pd.Series):
            ...
            shap_df = pd.DataFrame()
        # Identify columns for features and SHAP values
        feat_col = None
        shap_col = None
        if not shap_df.empty:
            cols_lower = {c.lower(): c for c in shap_df.columns}
            for c in ["feature", "name", "variable"]:
                if c in shap_df.columns:
                    feat_col = c
                    break
                if c.lower() in cols_lower:
                    feat_col = cols_lower[c.lower()]
                    break
            for c in ["shap", "impact", "value", "shap_value"]:
                if c in shap_df.columns:
                    shap_col = c
                    break
                if c.lower() in cols_lower:
                    shap_col = cols_lower[c.lower()]
                    break
        if shap_col is None or feat_col is None:
