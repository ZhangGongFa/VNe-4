...
def render(feats_df: pd.DataFrame, raw_df: pd.DataFrame) -> None:
    ...
    with st.expander("📄 Xem danh sách công ty" if lang == 'vi' else "📄 View company list", expanded=False):
        """Show a list of tickers after filters are applied.  We no longer display the list of
        input variables as the dashboard should remain clean and focused on core KPIs."""
        if num_companies:
    ...
    # in the selected year and converting from VND to bn VND and then to bn USD.
    try:
        total_assets_vnd = 0.0
        if 'TOTAL ASSETS (Bn. VND)' in year_filtered_df.columns:
            # The column name suggests "Bn VND" but values are raw VND, so we convert properly
        ...
        total_assets_vnd = 0.0
        total_assets_bn_vnd = 0.0
        total_assets_bn_usd = 0.0
        gdp_usd = None
        market_share = 0.0
    ...
    # Show numerical values of total assets and GDP for context, using bn units for readability
    if lang == 'vi':
        assets_text = f"Tổng tài sản: {total_assets_bn_vnd:,.2f} bn VND (≈ {total_assets_bn_usd:,.2f} bn USD)"
        gdp_text = f"GDP: {gdp_usd:,.2f} bn USD"
    else:
        assets_text = f"Total assets: {total_assets_bn_usd:,.2f} bn USD (≈ {total_assets_bn_vnd:,.2f} bn VND)"
        gdp_text = f"GDP: {gdp_usd:,.2f} bn USD"
